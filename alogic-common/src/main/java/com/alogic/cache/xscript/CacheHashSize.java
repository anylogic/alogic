 ...
 * @since 1.6.11.29
 */
public class CacheHashSize extends CacheObjectOperation{
	...
	public CacheHashSize(String tag, Logiclet p) {
		super(tag, p);
	}
	...
	protected void onExecute(CacheObject cache, Map<String, Object> root,
		...
		if (StringUtils.isNotEmpty(id)){
			ctx.SetValue(
				id, 
				String.valueOf(
					cache.hLen(PropertiesConstants.transform(ctx, $group, CacheObject.DEFAULT_GROUP))
				)
			);
		}
	}
